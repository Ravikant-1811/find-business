...
/* =========================================
   APP VIEW – ADD BUSINESS UI FIX
========================================= */

/* Only affect Android App */
.app-view {
...
/* Center title nicely */
.app-view .form-header {
  margin-bottom: 1.25rem !important;
}

.app-view .form-header h2 {
  font-size: 1.4rem !important;
  text-align: center !important;
}

.app-view .form-header p {
  font-size: 0.9rem !important;
  text-align: center !important;
}

/* Inputs – bigger & app friendly */
.app-view .form-input,
...
.app-view .section-title h3 {
  font-size: 1rem !important;
}

/* Reduce excessive gaps */
.app-view .form-section {
  margin-bottom: 1.25rem !important;
}

/* Submit button – app style */
.app-view .submit-btn {
  width: 100% !important;
...
<style>
/* ===============================
   APP MODE – NO ANIMATION, NO MOTION
================================= */
.app-view * {
...
<div class="success-alert" id="successAlert" style="display: none;">
    ...
    <!-- Success Alert -->
<div id="successAlert" style="display: none; position: fixed; top: 20px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 20px 30px; border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.2); z-index: 9999; align-items: center; gap: 15px;">
    <i class="fas fa-check-circle" style="font-size: 24px;"></i>
    <div>
        <strong>Success!</strong>
        <p style="margin: 5px 0 0;">Your business has been submitted. Redirecting...</p>
    </div>
</div>
...
(function() {
    ...
    async function handleFormSubmit(e) {
        ...
        // Get form data as URL encoded string (THIS IS THE FIX!)
        ...
        formData.append('business_name', form.querySelector('[name="business_name"]').value);
        formData.append('category', form.querySelector('[name="category"]').value);
        formData.append('business_place', form.querySelector('[name="business_place"]').value);
        ...
        try {
            // Send to PHP with correct headers
            ...
            if (result.status === 'success') {
                // Show success alert
                showSuccessAlert(result.reference);
                ...
                // Show errors
                showErrors(result.errors || ['Unknown error occurred']);
            }
     ...
     * Show Success Alert
     */
    function showSuccessAlert(reference) {
        // Remove existing alerts
        const existingAlert = document.querySelector('.success-alert');
        if (existingAlert) existingAlert.remove();

        const html = `
            <div class="success-alert" id="successAlert" style="
                display: flex;
                align-items: center;
                gap: 15px;
                background: linear-gradient(135deg, #10b981 0%, #059669 100%);
            ...
            ">
                ...
                <div>
                    <h4 style="margin: 0 0 5px 0;">Business Submitted Successfully!</h4>
                    <p style="margin: 0; opacity: 0.9;">Reference: <strong>${reference}</strong></p>
                </div>
    ...
    function showErrors(errors) {
        ...
        // Create error HTML
        let html = `
            <div class="error-alert" id="errorAlert" style="
                display: flex;
                align-items: flex-start;
                gap: 15px;
                background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%);
     ...
     * Event Listeners
     */
    if (navbar) window.addEventListener('scroll', handleScroll);
